...
abstract class Model
{
    ...
    public function minimun(?string $terms = null, string $columns = "*")
    {
        $this->query = "SELECT {$columns} FROM (select {$columns}, 
        row_number() over (order by id DESC) {$this->entity}
        from {$this->entity}) tt WHERE {$terms}";
        return $this;
    }
    public function minimunCategory(?string $terms = null, int $category, string $columns = "*")
    {
        $this->query = "SELECT {$columns} FROM (select {$columns}, 
        row_number() over (order by id DESC) {$this->entity}
        from {$this->entity} WHERE category_id = {$category}) tt WHERE {$terms}";
        return $this;
    }
